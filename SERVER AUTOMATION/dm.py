...
def passwdbox(h,w,filepath):
	os.system("dialog --ascii-lines  --title 'User Authentication' --clear --insecure --passwordbox 'Enter password' "+h+" "+w+" 2> "+filepath)
...
def dirbox(init_dir_path, filepath):
	os.system("dialog --ascii-lines  --title 'Select a Directory' --dselect "+init_dir_path+" 25 60 2> "+filepath)

def filebox(init_dir_path, filepath):
	os.system("dialog --ascii-lines  --title 'Select a File' --fselect "+init_dir_path+" 25 60 2> "+filepath)

def editbox(filepath,h,w):
	os.system("dialog --ascii-lines  --fselect "+filepath+" 25 60 2> "+filepath)

def tailbox(filepath,h,w):
	os.system("dialog --ascii-lines  --tailbox "+filepath+" "+h+" "+w)

def textbox(filepath,h,w):
	os.system("dialog --ascii-lines  --textbox "+filepath+" "+h+" "+w)
